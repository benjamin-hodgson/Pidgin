...
namespace Pidgin
{
    ...
    internal sealed class SequenceParser<TToken, T> : Parser<TToken, IEnumerable<T>>
    {
        ...
        internal sealed override bool TryParse(ref ParseState<TToken> state, ICollection<Expected<TToken>> expecteds, [MaybeNullWhen(false)] out IEnumerable<T> result)
        {
            ...
            for (var i = 0; i < _parsers.Length; i++)
            {
                ...
                var success = p.TryParse(ref state, expecteds, out ts[i]!);
        ...
        where TEnumerable : IEnumerable<TToken>
    {
        ...
        internal sealed override bool TryParse(ref ParseState<TToken> state, ICollection<Expected<TToken>> expecteds, [MaybeNullWhen(false)] out TEnumerable result)
        {
